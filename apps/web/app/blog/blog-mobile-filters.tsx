...
interface BlogMobileFiltersProps {
  allPosts: Post[];
  ...
  return (
    <>
      ...
      <button
        ...
        className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-white bg-neutral-800 border border-neutral-700 rounded-lg hover:bg-neutral-700 transition-all"
      >
        <Filter className="h-4 w-4" />
        <span>Filters & Search</span>
        {activeFilterCount > 0 && (
          <span className="ml-1 px-2 py-0.5 text-xs font-semibold text-primary-300 bg-primary-900/50 border border-primary-700 rounded-full">
            {activeFilterCount}
          </span>
        )}
      </button>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          {/* Modal Content */}
          <div className="absolute inset-x-0 bottom-0 max-h-[85vh] bg-neutral-900 border-t border-neutral-700 rounded-t-2xl overflow-hidden animate-slide-up">
            {/* Header */}
            <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-4 bg-neutral-800 border-b border-neutral-700">
